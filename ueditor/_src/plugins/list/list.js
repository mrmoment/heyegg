...
//                                    range.setEnd(pre, pre.childNodes.length).shrinkBoundary().collapse().select(true);
        ...
        baidu.editor.commands['insertunorderedlist'] = {
            execCommand : function( command, style ) {
                ...
                if(preList && preList.tagName.toLowerCase() == tag && domUtils.getStyle(preList,'list-style-type') == style){
                    domUtils.moveChild(list,preList);
                }
                var nextList = list.nextSibling;
                if(nextList && nextList.tagName.toLowerCase() == tag && domUtils.getStyle(nextList,'list-style-type') == style){
                    domUtils.moveChild(nextList,preList);
